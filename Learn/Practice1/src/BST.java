...
public class BST<T extends Comparable<T>> implements Iterable<T> {
	public static void main(String[] args) {
		...
		bst.restore(
				new Integer[] { 11, 8, 6, 4, 7, 10, 19, 43, 31, 29, 37, 49 },
	...
	private Node<T> restore(T[] pre, int preL, int preR, T[] in, int inL,
			int inR) {
		if (preL <= preR) {
			int count = 0;
			...
			tmp.left = restore(pre, preL + 1, preL + count, in, inL, inL
					+ count - 1);
			tmp.right = restore(pre, preL + count + 1, preR, in, inL + count
					+ 1, inR);
			return tmp;
		} else
			return null;
